...
async function fetchSummary() {
    try {
        ...
        if (el('avg-sale-label')) {
            el('avg-sale-label').innerHTML = `Avg sale: Rs ${(data.average_sale || 0).toLocaleString()}`;
        }
...
// --- Fetch Sales Stats (matches /api/dashboard/sales-stats) ---
async function fetchSalesStats() {
    try {
        const data = await api.get('dashboard/sales-stats');
