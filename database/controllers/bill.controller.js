    ...
    try {
        ...
        if (items && items.length > 0) {
            ...
            for (const item of items) {
                ...
                // If product doesn't exist and it's a buy bill, create it
                if (!productData && bill_type === 'buy') {
                    console.log(`Creating new product "${item.product_name}" for purchase bill`);
                ...
                if (productError && bill_type === 'sell') {
                    console.warn(`Product "${item.product_name}" not found for sell bill, skipping stock update`);
                    continue;
                }
